    ...
    <div className="space-y-6 animate-in-fade">
      ...
      <div className="flex flex-col sm:flex-row gap-3 animate-in-slide-up" style={{ animationDelay: '0.1s' }}>
        ...
        <Button variant="outline" className="h-14 sm:h-auto px-4 rounded-2xl border-clinical-200 bg-white">
          ...
          <span className="font-bold text-xs uppercase tracking-widest text-clinical-600">Advanced Filter</span>
        </Button>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {filteredPatients?.map((patient, idx) => (
              <div key={patient.id} className="animate-in-slide-up" style={{ animationDelay: `${0.15 + (idx * 0.05)}s` }}>
...
function PatientCard({ patient }: { patient: Patient }) {
  ...
  return (
    ...
    >
      <Card className="h-full border-clinical-200/60 hover:border-clinical-400 hover:shadow-xl hover:shadow-clinical-900/5 transition-all duration-300 cursor-pointer overflow-hidden relative active:scale-[0.99]">
          ...
          <div className="flex items-stretch h-full">
            ...
            <div className="p-6 flex-1 min-w-0">
              <div className="flex items-start gap-4">
                ...
                <div className="h-8 w-8 rounded-full bg-clinical-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0 shrink-0 shadow-sm border border-clinical-100">
